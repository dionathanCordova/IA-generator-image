...
import { CLASS_CONFIGS, ITEMS_BY_CLASS } from "./constants";
import { RPGItem, Rarity, CharacterClass } from "./types";
import { generateItemImage } from "./services/geminiService";

export default function App() {
  const [activeClass, setActiveClass] = useState<CharacterClass>(CharacterClass.WARRIOR);
  const [activeRarity, setActiveRarity] = useState<Rarity>(Rarity.COMMON);
  const [generatedImages, setGeneratedImages] = useState<Record<string, string>>({});
  const [loadingItems, setLoadingItems] = useState<Record<string, boolean>>({});
  const [error, setError] = useState<string | null>(null);
  const [previewImage, setPreviewImage] = useState<{ url: string; name: string } | null>(null);

  const rarityDisplayNames: Record<Rarity, string> = {
    [Rarity.COMMON]: "Comum",
    [Rarity.INTERMEDIATE]: "Intermediário",
    [Rarity.EXCELLENT]: "Excelente",
    [Rarity.EPIC]: "Épico",
    [Rarity.LEGENDARY]: "Lendário",
  };

  const currentClassConfig = CLASS_CONFIGS[activeClass];
  const currentItems = ITEMS_BY_CLASS[activeClass][activeRarity] || [];
        ...
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-1 bg-white/5 p-1 rounded-full border border-white/5">
            {Object.entries(CLASS_CONFIGS).map(([key, config]) => {
              const Icon = key === 'Warrior' ? Sword : 
                           key === 'Mage' ? Wand2 : 
                           key === 'Hunter' ? Target : 
                           key === 'Lord' ? Crown : Skull;
              return (
                <button
                  key={key}
                  onClick={() => setActiveClass(key as CharacterClass)}
                  className={`px-4 py-2 rounded-full text-xs font-bold uppercase tracking-widest transition-all duration-300 flex items-center gap-2 ${
                    activeClass === key
        ...
        <div className="flex flex-wrap items-center justify-center gap-2 mb-12">
          {Object.values(Rarity).map((rarity) => (
            <button
              ...
              className={`px-6 py-2.5 rounded-xl text-xs font-bold uppercase tracking-[0.2em] transition-all duration-500 border ${
                activeRarity === rarity
        ...
        >
          <div className="flex items-center gap-3 mb-4">
            <span className="px-3 py-1 rounded-full bg-white/5 border border-white/10 text-[10px] font-bold uppercase tracking-widest text-white/60">
              {currentClassConfig.name} • {rarityDisplayNames[activeRarity]}
            </span>
                    ...
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      {item.rarity && (
                        <span className={`px-1.5 py-0.5 rounded text-[8px] font-bold uppercase tracking-widest ${
                          item.rarity === Rarity.COMMON ? 'bg-emerald-500/10 text-emerald-500/60' :
                          item.rarity === Rarity.INTERMEDIATE ? 'bg-blue-500/10 text-blue-500/60' :
                          item.rarity === Rarity.EXCELLENT ? 'bg-amber-500/10 text-amber-500/60' :
                          item.rarity === Rarity.EPIC ? 'bg-purple-500/10 text-purple-500/60' :
                          'bg-orange-500/10 text-orange-500/60'
                        }`}>
                          {rarityDisplayNames[item.rarity]}
                        </span>
                      )}
                    </div>
